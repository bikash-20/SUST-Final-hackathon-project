...
LiquiGuard is a **synthetic-data-only prototype**. It does not connect to a real wallet, move funds, block an account, or declare fraud. All transactions, demand surges, and provider drains are produced inside the same FastAPI process running against an indexed PostgreSQL 16 instance — never from a static JSON fixture, never from a flat-file replay.
